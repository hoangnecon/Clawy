...
2. **Onboards the Custom Provider**: It runs `openclaw onboard` using the `--custom-provider-id openai` flag. By masquerading as Native OpenAI (rather than a generic custom API), OpenClaw inherits robust routing logic while pointing at `http://localhost:8046/v1`.
